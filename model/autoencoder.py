...
def make_tests(load_data: (), tag=""):
    ...
    train_loader = DataLoader(
        TensorDataset(train_tensor, train_tensor), batch_size=128, shuffle=True
    )
    ...
    )  # optymalizator - minimalizacja obliczonego wyżej błędu
    ...
    with torch.no_grad():  # nie trzeba tego pamiętać do nauki gradientów
        train_reconstructions = model(
            train_tensor
        )  # sprawdzenie jak wygląda norma - stworzenie "mapy normalności"
    ...
    with torch.no_grad():
        ...
        train_errors = torch.mean(
            (train_tensor - train_reconstructions) ** 2, dim=1
        ).numpy()
        test_errors_full = torch.mean(
            (test_tensor - test_reconstructions) ** 2, dim=1
        ).numpy()
