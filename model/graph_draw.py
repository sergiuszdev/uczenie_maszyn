...
def plot_tradeoff_analysis(results_accumulator, percentiles, filename='results/analiza_percentyli.png'):
    ...
    plt.plot(sorted_percentiles, y_recall_avg, label='Średni Recall (Czułość)', 
             color='blue', marker='o', linestyle='-', linewidth=2)
    
    plt.plot(sorted_percentiles, y_precision_avg, label='Średnia Precision (Precyzja)', 
             color='green', marker='s', linestyle='-', linewidth=2)
    
    plt.plot(sorted_percentiles, y_f1_avg, label='Średni F1-Score', 
             color='red', marker='^', linestyle='--', alpha=0.7)

    plt.title('Zmiana wartości parametrów Recall i Precision w zależności od ustalonego percentyla', fontsize=14)
...
def plot_roc_curve(y_true, y_scores, filename='results/roc_curve.png'):
...
def plot_error_histogram(y_true, errors, threshold=None, filename='results/histogram_bledow.png'):
    ...
    sns.histplot(errors_normal, color='green', label='Ruch normalny', kde=False, bins=100, alpha=0.3, element="step")
    sns.histplot(errors_attack, color='red', label='Atak', kde=False, bins=100, alpha=0.3, element="step")

    if threshold is not None:
        plt.axvline(threshold, color='black', linestyle='--', linewidth=2, label=f'Próg ({threshold:.5f})')
...
def plot_preprocessing_effect(df, feature_name, filename='results/preprocessing_effect.png'):
    if feature_name not in df.columns:
        print(f"Błąd: Nie znaleziono kolumny '{feature_name}' w danych. Wykres nie zostanie wygenerowany")
        return
    ...
    sns.histplot(processed_data, bins=50, ax=axes[1], color='blue', kde=True, stat="density")
...
def plot_feature_separability(df, y_true, feature_name, apply_log=True, filename='results/feature_separability.png'):
    if feature_name not in df.columns:
        print(f"Błąd: Nie znaleziono kolumny '{feature_name}' w danych. Wykres nie zostanie wygenerowany")
        return
