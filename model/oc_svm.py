...
def run_ocsvm_cross_validation(data_loader_func, tag="ocsvm_cv", n_splits=5):
    tag_full = f"{tag}_sgd"
    ...
    print("\n" + "="*105)
    print(f"WYNIKI CV (OC-SVM SGD) - {tag_full}")
    print("="*105)
    ...
    plot_tradeoff_analysis(results_accumulator, possible_percentiles, file_tag=tag_full)
    plot_roc_curve(y_true_global, test_scores_global, file_tag=tag_full)
    ...
    save_results_to_csv(results_accumulator, avg_thresholds_map, file_tag=tag_full)
